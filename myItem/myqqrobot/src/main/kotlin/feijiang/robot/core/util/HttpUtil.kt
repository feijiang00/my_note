package feijiang.robot.core.util
...
//            .setProxy(HttpHost("127.0.0.1", 5533))
    ...
    private fun request(method: HttpMethod, requestEntity: RequestEntity.() -> Unit): ResponseEntity {
        ...
        val httpRequest: HttpRequestBase = when (method) {
            ...
            HttpMethod.Post -> HttpPost(uri).also {
            ...
            HttpMethod.Put -> HttpPut(uri).also {
                if (entity.json.isNotBlank()) {
                    it.entity = StringEntity(entity.json)
                    it.setHeader(HttpHeaders.CONTENT_TYPE, ContentType.APPLICATION_JSON.mimeType)
                }
    ...
    fun get(requestEntity: RequestEntity.() -> Unit): ResponseEntity {
        return request(HttpMethod.Get, requestEntity)
    }

    fun post(requestEntity: RequestEntity.() -> Unit): ResponseEntity {
        return request(HttpMethod.Post, requestEntity)
    }

    fun delete(requestEntity: RequestEntity.() -> Unit): ResponseEntity {
        return request(HttpMethod.Delete, requestEntity)
    }

    fun put(requestEntity: RequestEntity.() -> Unit): ResponseEntity {
        return request(HttpMethod.Put, requestEntity)
    }
     ...
     * 解析腾讯接口返回结果的一个方法
     */
    fun getJson(url: String, separator: String): JSONObject {
...
class ResponseEntity {
    ...
    fun getJSONResponse(): JSONObject? = if (response.isBlank()) null else JSON.parseObject(response)
